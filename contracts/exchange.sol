...
contract Exchange {
    ...
    function makeOrder(address _tokenBuy, uint256 _amountBuy, address _tokenSell, uint256 _amountSell) public {
    ...
    function cancelOrder(uint256 _id) public {
        ...
        emit Cancel(_order.id, _order.user, _order.tokenBuy, _order.amountBuy, _order.tokenSell, _order.amountSell, block.timestamp);
    }

    function fillOrder(uint256 _id)  public {
        require(_id > 0 && _id <= orderCount, 'Order does not exist');
        require(!filledOrder[_id] && !cancelledOrder[_id], "Order is already filled or cancelled");
        _Order storage _order = orders[_id];
        _trade(_order.id, _order.user, _order.tokenBuy, _order.amountBuy, _order.tokenSell, _order.amountSell);
        filledOrder[_order.id] = true;
    }

    function _trade(uint256 _id, address _user, address _tokenBuy, uint256 _amountBuy, address _tokenSell, uint256 _amountSell) internal {
        ...
        emit Trade(_id, _user, _tokenBuy, _amountBuy, _tokenSell, _amountSell, msg.sender, block.timestamp);
